...
int isBetween(int x, int h, int l) {
  ...
  int xminush = (((x + (~h + 1)) >> 31) & 1); // 1 if h > x
  int lminusx = (((l + (~x + 1)) >> 31) & 1); // 1 if x > l

  int notx = (!xsign);
  int notxmh = (!xminush);
  int notlmx = (!lminusx);

  int xminushOF = (notx & hsign & xminush) | (xsign & !hsign & notxmh);
  int lminusxOF = ((!lsign) & xsign & lminusx) | (lsign & notx & notlmx);

  int xlessh = ((xminush & (!xminushOF)) | (notxmh & xminushOF)); // 1 if h > x after considering overflow

  int llessx = ((lminusx & (!lminusxOF)) | (notlmx & lminusxOF)); // 1 if x > l after considering overflow

  return ( xlessh & llessx ); // h > x and x > l
}
/* 
...
int rotate(int x, int w) {
  int highBit = ((x >> 31) & 1);
  int lowBit = (x & 1);
  int left_rotate = ((x << w) | (w & highBit));
  int lowBitShifted = lowBit << 31;
  int right_rotate = ((x >> !w) & ~(1 << 31)) | lowBitShifted;

  int mask = ((w << 31) >> 31);

  int left_answer = left_rotate & mask;
  int right_answer = (right_rotate & ~mask);

  int result = left_answer | right_answer;
  return result;
}
/* 
...
int negVal(int x) {
  ...
  int mask = (x >> 31);
  int b4mask = (~x + 1);
  int xpositive = b4mask & ~mask;
  int xnegative = x & mask;
...
int twocomp2onecomp(int x) {
  ...
  int combo = (highBit & tempx);

  int negativeX = x & mask;
  int negative = (~negativeX  + combo) | (!tempx << 31);
